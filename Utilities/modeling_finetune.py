...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        #qkv = torch.matmul(x, torch.transpose(torch.tensor(self.qkv.weight), 0, 1))
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=48, in_chans=1, out_chans=8, num_classes=1000, embed_dim=48, depth=12,
        ...
        print('Class token shape: ', self.cls_token.shape)

        # self.mask_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, 128 + 1, embed_dim), requires_grad=True)
            #self.pos_embed = nn.Parameter(torch.zeros(1, 64 + 1, embed_dim), requires_grad=True)
...
@register_model
def labram_base_patch200_200(pretrained=True, use_abs_pos_emb = True, **kwargs):
...
def labram_large_patch200_200(pretrained=False, **kwargs):
    model = NeuralTransformer(
        patch_size=200, embed_dim=48, depth=24, num_heads=16, mlp_ratio=4, out_chans=16, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_abs_pos_emb=True, **kwargs)
    model.default_cfg = _cfg()
    return model
...
def labram_huge_patch200_200(pretrained=False, **kwargs):
    model = NeuralTransformer(
        patch_size=200, embed_dim=48, depth=48, num_heads=16, mlp_ratio=4, out_chans=32, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_abs_pos_emb=True, **kwargs)
    model.default_cfg = _cfg()
    return model
