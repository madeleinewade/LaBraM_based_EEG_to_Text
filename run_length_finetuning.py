...
def get_args():
    ...
    parser.add_argument('--output_dir', default='/content/drive/MyDrive/LaBraM-main/',
...
def get_dataset(args):
    if args.dataset == 'TUAB':
        ...
        y_train = [int(y[1] +1) for y in y_train]
        y_val = [int(y[1]+1) for y in y_val]
        y_test = [int(y[1]+1) for y in y_test]

        assert max(y_train) < 22
        assert max(y_val) < 22
        assert max(y_test) < 22
