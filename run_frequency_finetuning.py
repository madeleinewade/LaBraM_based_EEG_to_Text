...
def get_args():
    ...
    parser.add_argument('--input_size', default=276, type=int,
                        help='EEG input size')
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='/content/drive/MyDrive/LaBraM-main/checkpoints/labram-base.pth',
    ...
    # Dataset parameters
    parser.add_argument('--nb_classes', default=58, type=int,
                        help='number of the classification types')

    parser.add_argument('--output_dir', default='/content/drive/MyDrive/LaBraM-main/frequency_finetune/',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='/content/drive/MyDrive/LaBraM-main/frequency_finetune/',
    ...
    parser.add_argument('--dataset', default='Murphy2022', type=str,
...
def get_dataset(args):
    if args.dataset == 'TUAB':
    ...
    elif args.dataset == 'Murphy2022':
        
        with open('/content/drive/MyDrive/LaBraM-main/x_train.pkl', 'rb') as f:
            x_train = pkl.load(f)

        with open('/content/drive/MyDrive/LaBraM-main/y_train.pkl', 'rb') as f:
            y_train = pkl.load(f)

        with open('/content/drive/MyDrive/LaBraM-main/x_val.pkl', 'rb') as f:
            x_val = pkl.load(f)

        with open('/content/drive/MyDrive/LaBraM-main/y_val.pkl', 'rb') as f:
            y_val = pkl.load(f)

        with open('/content/drive/MyDrive/LaBraM-main/x_test.pkl', 'rb') as f:
            x_test = pkl.load(f)

        with open('/content/drive/MyDrive/LaBraM-main/y_test.pkl', 'rb') as f:
            y_test = pkl.load(f)

        y_train = [int(y[2]) for y in y_train]
        y_val = [int(y[2]) for y in y_val]
        y_test = [int(y[2]) for y in y_test]

        assert max(y_train) < 58
        assert max(y_val) < 58
        assert max(y_test) < 58
        ...
        ch_names = ['FP1', 'FZ', 'F3', 'F7', 'FC5',
       'FC1', 'C3', 'T7', 'CP5', 'CP1', 'PZ', 'P3', 'P7', 'O1', 'OZ',
       'O2', 'P4', 'P8', 'CP6', 'CP2', 'CZ', 'C4', 'T8', 'FC6',
       'FC2', 'F4', 'F8', 'FP2', 'AF7', 'AF3', 'F1', 'F5', 'FT7', 'FC3',
       'C1', 'C5', 'TP7', 'CP3', 'P1', 'P5', 'PO7', 'PO3', 'POZ', 'PO4', 'PO8',
       'P6', 'P2', 'CPZ', 'CP4', 'TP8', 'C6', 'C2', 'FC4', 'FT8', 'F6', 'AF8',
       'AF4', 'F2', 'FCZ', 'FT9', 'FT10', 'AFZ', 'TP9', 'TP10']
        args.nb_classes = 58
...
def main(args, ds_init):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(f'Max accuracy val: {max_accuracy:.4f}, max accuracy test: {max_accuracy_test:.4f}')
            if log_writer is not None:
